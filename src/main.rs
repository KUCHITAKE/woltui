...
struct App<'a> {
    ...
    machines: StatefulList<(String, String)>,
    status_message: String,
    textarea: TextArea<'a>,
    editing_name: String,
    editing_mac: String,
    mac_regex: Regex,
    popup_time: Option<Instant>,
}

impl<'a> App<'a> {
    fn new() -> App<'a> {
        let sm = Machine::new(Main).as_enum();
        let mut app = App {
            state_machine: sm,
    ...
    fn load_machines(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let config = crate::config::read_config(config_path.as_path())?.machines;
    ...
    fn add_machine(&mut self, name: &str, mac: &str) -> Result<(), Box<dyn std::error::Error>> {
        self.machines
            .items
    ...
    fn delete_machine(&mut self, index: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn save_machines(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        let config_path = match dirs::home_dir() {
            Some(home_dir) => Ok(home_dir.join(".wol").join("config")),
            None => Err(Box::new(std::io::Error::new(
                std::io::ErrorKind::NotFound,
